...
class ImportEpisodeJob extends BprsContainerAwareJob {
    ...
    public function perform()
    {
        ...
        if (filter_var($this->args['overwrite'], FILTER_VALIDATE_BOOLEAN) || $episode->getSeries() == null) {
            ...
            if ($serializing_schema) {
                $response = $this->media_service->getResponse(
                    $episode->getKeychain(),
                    MediaService::ROUTE_EPISODE,
                    ['uniqID' => $this->args['uniqID'],
                    'group' => $serializing_schema]
                );
            } else {
                $response = $this->media_service->getResponse(
                    $episode->getKeychain(),
            ...
            if ($response->getStatusCode() == 200) {
                ...
                $remote_episode = $this->serializer->deserialize(
                    $response->getBody(),
                    $episode_class,
                    'json'
                );
                $series = $this->media_service->getSeries(
                    $remote_episode->getSeries()->getUniqID()
                );
                if (!$series) {
                    $series = $this->importSeries(
                        $remote_episode->getSeries()->getUniqID(),
                        $episode->getKeychain()
                    );
                }
                ...
                $this->logbook->error(
                    'okto_media.episode_import_episode_error',
                    [],
                    $this->args['uniqID']
                );
            }
            ...
            $this->logbook->info(
                'okto_media.episode_overwrite_not_allowed',
                [],
                $this->args['uniqID']
            );
        }
        $this->logbook->info(
            'okto_media.end_episode_import',
            [],
            $this->args['uniqID']
        );
    }
    ...
    private function importSeries($uniqID, $keychain)
    {
        ...
        if ($response->getStatusCode() == 200) {
            ...
            $series = $this->serializer->deserialize(
                $response->getBody(),
                $series_class,
                'json'
            );
            ...
            $this->logbook->error(
                'okto_media.episode_import_series_error',
                [],
                $this->args['uniqID']
            );
        }
